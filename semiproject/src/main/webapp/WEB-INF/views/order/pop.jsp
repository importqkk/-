	   ...
	   <form id="myForm" action="/order/popup" method="post">
	     <c:forEach var="allInfo" items="${allInfo}" varStatus="loop">
	        	<div class="mt-20" style="border: 2px solid rgb(119,107,255) ; width: 500px; padding: 10px;">
		            ...
		            <div>
		                <button class="select-btn form-btn small2 positive" type="submit" data-index="${loop.index}">선택</button>
		                <button class="delete-btn form-btn small2 positive" type="button" data-index="${loop.index}">삭제</button>
		            </div>
		        </div>
  		</c:forEach>
	</form>
